...
//		                }
	...
	private void getFreqInput(){
		...
		this.freqThree = this.f3.getText();
	}
	...
	private boolean isInputValid() throws IOException{
        ...
        if ((f1.getText() == null || f1.getText().length() == 0 || !isDouble(f1.getText()))
        		...
        		|| (f3.getText() == null || f3.getText().length() == 0 || !isDouble(f3.getText()))) {
	...
	private void computeVariousIntermodulation(){
		...
		double three = new Double(this.freqThree);
		
		DecimalFormat df = new DecimalFormat("#.##"); 
		
		Double result = ((2*One) + two) - three;
		results.add(new TaskModel("3", "((2 * " + One +") + " + two + ") - " + three + "", String.valueOf(df.format(result)) + " MHz" ));
		
		result = ((One) + (2*two)) - (2*three);
		results.add(new TaskModel("3", "((" + One +") + 2 * " + two + ") - (2 * " + three + ")", String.valueOf(df.format(result)) + " MHz" ));
		
		result = ((2*One) - (two)) + (2*three);
		results.add(new TaskModel("3", "((2 * " + One +") - " + two + ") + (2 * " + three + ")", String.valueOf(df.format(result)) + " MHz" ));
		
		result = ((One) - (2*two)) + (2*three);
		results.add(new TaskModel("3", "((" + One +") - (2 * " + two + ") + (2 * " + three + ")", String.valueOf(df.format(result)) + " MHz" ));
		
		result =  ((2*two) + three) - (2*One);
		results.add(new TaskModel("3", "((2 * " + two +") + " + three + ") - (2 * " + One + ")", String.valueOf(df.format(result)) + " MHz" ));
		
		result = ((2*three) + two) - (2*One);
		results.add(new TaskModel("3", "((2 * " + three +") + " + two + ") - (2 * " + One + ")", String.valueOf(df.format(result)) + " MHz" ));
